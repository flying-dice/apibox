...
agent: claude
live: false
status: Reply, examples, variables and schemaFormat all landed
progress: 100
...
- [x] Parse message `examples` (UI already exists)
- [x] Parse server `variables` (UI already exists)
- [x] Model and parse operation `reply` + `replyAddress`
- [x] Detect `schemaFormat`; warn and skip JSON Schema walking for non-JSON-Schema payloads
- [x] Parse operation-level `security` and `tags`
- [x] Keep channels with zero operations in the output
- [x] Render `info.tags` for AsyncAPI as OpenAPI already does
- [x] Tests
