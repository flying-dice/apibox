 ...
 */
function needsFreshShell(argv) {
    if (argv[0] !== 'build')
        return false;
    return !argv.some((argument) => argument === '--asset-dir' || argument.startsWith('--asset-dir='));
}
function buildViewer() {
    ...
    if (result.status !== 0) {
        throw new Error('Failed to build the viewer shell. Fix the build, or pass --asset-dir to point at a ' +
